    ...
    <div className="p-8 lg:p-12 animate-in fade-in zoom-in duration-500 min-h-full flex flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-arena-muted text-xs font-bold tracking-[0.3em] uppercase">
              {reviewer?.name || reviewer?.display_name || 'EVALUATOR'} • Active Duty
            </p>
          </div>
        </div>

        <div className="flex items-center gap-3 bg-arena-rose/10 border border-arena-rose/30 px-6 py-3 rounded-2xl shadow-wine-glow">
          <Award className="w-5 h-5 text-arena-rose animate-pulse" />
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-arena-rose uppercase tracking-widest">Active Evaluation</span>
            <span className="text-lg font-black text-white uppercase tracking-widest">Round {activeRound}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        ...
        <div className="glass-premium p-6 border-l-4 border-l-yellow-500/50 flex flex-col justify-center">
          ...
          <span className="text-4xl font-black text-yellow-100">{teams.filter(t => t.status !== 'Completed').length}</span>
        </div>
        <div className="glass-premium p-6 border-l-4 border-l-green-500/50 flex flex-col justify-center">
          ...
          <span className="text-4xl font-black text-green-100">{teams.filter(t => t.status === 'Completed').length}</span>
        </div>
      </div>
      ...
      <div className="glass-premium flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar p-6">
          {loading ? (
             <div className="h-full flex items-center justify-center text-arena-rose animate-pulse">Syncing...</div>
          ) : (
            <div className="flex flex-col gap-4">
              {teams.map(team => (
                <div key={team.id} className="flex flex-col md:flex-row items-center justify-between p-5 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-arena-rose/30 transition-all group">
                  <div className="flex items-center gap-6 mb-4 md:mb-0 w-full md:w-auto">
                    <div className="w-12 h-12 bg-black/40 rounded-lg flex items-center justify-center flex-shrink-0 border border-white/5">
                      <span className="text-xs font-black text-arena-rose">{team.id.split('-')[1]}</span>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                    ...
                    <button 
                      onClick={() => navigate(`/reviewer/evaluation/${team.id}`)}
